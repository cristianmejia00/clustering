...
## Create the object
centrality_scores <- data.frame("X_N" = V(g1)$name,
...
## Closeness
if (addons$closeness_centrality) {
...
## Betweenness
if (addons$betweeness_centrality) {
...
centrality_scores <- merge(centrality_scores, dataset_minimal[,c("X_N", "UT")], by="X_N", all.y = TRUE)
...
write.csv(centrality_scores, 
          file = file.path(output_folder_reports, "stats", "additional_centrality_scores.csv"), 
          row.names = FALSE)
