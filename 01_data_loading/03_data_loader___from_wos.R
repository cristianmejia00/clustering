...
dataset_metadata <- list("query_id" = "Q311_innovativeness", 
...
if (Sys.info()["sysname"] == "Windows") {
  ...
  root_path_to   <- "/Users/cristian/Library/CloudStorage/GoogleDrive-cristianmejia00@gmail.com/My Drive"
} else {
  print("You are running R on a different operating system")
}

# Open a window to select the directory with the files to merge
myfolder = file.path(root_path_from, "imacros", "downloads", download_folder_name)
...
bibliometrics_folder <- file.path(root_path_to, "Bibliometrics_Drive")
...
network_folder <- file.path(bibliometrics_folder, 
                            dataset_metadata$query_id, 
                            paste("network_", format(Sys.Date(), "%Y-%m-%d"), sep = ""))
dir.create(network_folder, 
           showWarnings = FALSE)
