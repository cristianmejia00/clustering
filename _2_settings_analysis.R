...
settings$rp <- list(
  ...
  article_report_columns = list("X_C", "cluster_code", "AU", "PY", "DI", "TI", "AB", "Z9", "X_E", "DE", "SO", "WC", 
                                ...
                                'geopolitical_context_reasoning', 'outcome', 'technology_type', 'notes'
                                ),
  categorical_long_reports = list("AU", "WC", "SO", "Countries", "Institutions", "DE", "sentiment_factor", "ID", 
                                  'is_science_diplomacy', 'is_bibliometrics', 'policy_dimension',
                                  'framework_2025_categories_main',
                                  'framework_2025_categories_detailed', 
                                  'article_type', 'main_domain',
                                  'geographic_focus', 'power_dynamic',
                                   'diplomacy_goal', 
                                  'key_actors', 'global_commons',
                                   'geopolitical_context', 'outcome', 'technology_type'), # Columns in the dataset for long-form summary. These are also used for RCS.
  categorical_simple_wide_reports = list("PY", "sentiment_factor"), # Columns in the dataset without ';' for matrix type summary
  categorical_multi_wide_reports = list("WC", "Countries", "Institutions"), # Columns in the dataset with ';' for matrix type summary
  numerical_reports = list("PY", "Z9", "sentiment", "score")
                            # Numeric columns in the dataset for summary (min, max, mean, median, sd)
...
settings$stopwords$myStopWords <- unlist(list(
