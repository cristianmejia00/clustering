...
unifyer <- function(a_com, cl_threshold) {
  ...
  new_clusters <- sapply(a_com, function(x) {
    if (x > cl_threshold) {99}
    else (x)
  })
  # But if all are cluster 99, it means we did not make any sublustering
...
cl_threshold <- cl_selector(dataset_minimal$X_C, threshold = cno$threshold, size_lower_limit = cno$size_lower_limit, max_cluster = cno$max_clusters)
V(g1)$level0 <- dataset_minimal$level0 <- unifyer(dataset_minimal$X_C, cl_threshold)
...
for (i in c(1:cl_threshold,99)) {
  ...
  #if (vcount(subg) > cno$size_limit && resol_limit1[i] > 0) {
  if (vcount(subg) > cno$size_limit) {
    communi <- clusterize(subg, algorithm = cno$algor)
    temp_threshold <- cl_selector(communi, threshold = cno$threshold, size_lower_limit = cno$size_lower_limit, max_cluster = cno$max_clusters)
...
for (i in selected) {
  ...
  if ((vcount(subg) > cno$size_limit) && (resol_limit2[i] > 0)) {
    communi <- clusterize(subg, algorithm = cno$algor)
    temp_threshold <- cl_selector(communi, threshold = cno$threshold, size_lower_limit = cno$size_lower_limit, max_cluster = cno$max_clusters)
...
for (i in selected) {
  ...
  if (vcount(subg) > cno$size_limit && resol_limit3[i] > 0) {
    communi <- clusterize(subg, algorithm = cno$algor)
    temp_threshold <- cl_selector(communi, threshold = cno$threshold, size_lower_limit = cno$size_lower_limit, max_cluster = cnoo$max_clusters)
    new_communi <- unifyer(communi, temp_threshold)
