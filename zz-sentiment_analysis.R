...
sent_compound_mean <- tapply(dataset$sentiment, dataset$X_C_name, mean)
sent_compound_median <- tapply(dataset$sentiment, dataset$X_C_name, median)
...
bp <- ggplot(sentiment_long, aes(x = X_C, y = sentiment)) + geom_boxplot(width = 0.7, fill = "deepskyblue3") + xlab("Cluster") + ylab("Sentiment")
if (!(params$unit_of_analysis %in% c("topic", "topics", "cluster", "clusters"))) {
  bp + coord_flip()
}
if (K > 20) {
...
p <- ggplot(rcs_tmp, aes(x=sentiment_mean, y=cluster_size)) + geom_point() + xlab("Sentiment") + ylab("Cluster Size")
...
p <- ggplot(rcs_tmp, aes(x=sentiment_mean, y=cluster_year)) + geom_point() + xlab("Ave. Sentiment") + ylab("Ave. Year")
#p + geom_text(aes(label=cluster))
p + geom_text_repel(aes(label=cluster))
...
news_facet_year <- table(dataset$X_C, dataset$PY) %>% as.matrix() %>% as.data.frame.matrix()
...
# News per year
news_per_year <- table(dataset$PY) %>% as.matrix() %>% t() %>% as.data.frame.matrix()
...
sentiment_factor_per_year <- table(dataset$sentiment_factor, dataset$PY) %>% as.matrix() %>% as.data.frame.matrix()

# Ave. Sentiment per year
sentiment_per_year <- tapply(dataset$sentiment, dataset$PY, mean, na.rm = TRUE) %>% as.matrix() %>% t() %>% as.data.frame.matrix()
