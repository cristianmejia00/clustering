...
if ("DI" %in% colnames(article_report)) {
  article_report$DI <- DOI_to_URL(article_report$DI)
}
...
if (settings$params$type_of_dataset != "news" & settings$rp$top_documents != 0) {
...
if (settings$params$type_of_dataset == "patents") {
  setnames(article_report, change_names, patent_names, skip_absent = TRUE)
}
if (settings$params$type_of_dataset == "news") {
  setnames(article_report, change_names, news_names, skip_absent = TRUE)
}
