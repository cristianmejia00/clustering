...
# JOURNALS
################################################################################
stats_journals <- dataset$SO %>% as.character %>% table %>% sort(decreasing = TRUE) %>% data.frame() %>% setNames(c("Journals", "Articles"))
...
# COUNTRIES
################################################################################
stats_regions <- dataset$Countries %>% strsplit("; ") %>% unlist %>% table %>% sort(decreasing = TRUE) %>% data.frame() %>% setNames(c("Regions", "News"))
write.csv(stats_regions, file=file.path(output_folder_reports, "stats", "regions.csv"), row.names = FALSE)

ggplot(stats_regions[c(1:rp$top_items),], aes(x=Regions, y=News)) + 
  geom_bar(stat = "identity", width = 0.7, fill = "deepskyblue3") +  #, color = "black", fill = "white") +
  theme(axis.text=element_text(size=14)) +
  coord_flip() +
  scale_x_discrete(limits=rev)
...
if ("WC" %in% colnames(dataset)) {
  stats_WC <- dataset$WC %>% toupper %>% strsplit("; ") %>% unlist %>% table %>% sort(decreasing = TRUE) %>% data.frame() %>% setNames(c("Categories", "Articles"))
...
if ("DE" %in% colnames(dataset)) {
  stats_DE <- dataset$DE %>% tolower %>% strsplit("; ") %>% unlist %>% table %>% sort(decreasing = TRUE) %>% data.frame() %>% setNames(c("Keywords", "Articles"))
  #if (params$type_of_dataset == "news") { setnames(stats_DE, c("Keywords", "Articles"), c("Topic", "News"))}
...
if ("AU" %in% colnames(dataset)) {
  stats_AU <- dataset$AU %>% toupper %>% strsplit("; ") %>% unlist %>% table %>% sort(decreasing = TRUE) %>% data.frame() %>% setNames(c("Authors", "Articles"))
