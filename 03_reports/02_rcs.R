...
values = lapply(id_com, function(g) {
  ...
  max_degrees  <- cluster_data$"X_N"[which(degrees == dmax)][1] #get the id of the patents with highest digree #Remove "[1]" at the end to obtain multiple core papers per cluster
  rows         <- lapply(max_degrees, function(y) {
                        ...
                        hub_type1     <- if (params$type_of_dataset != "papers") {"ARTICLE"} else {toupper(cluster_data$"DT"[which(cluster_data$"X_N"==y)])} #Whether a paper was classified as Review by WOS
                        hub_type2     <- if (grepl("overview|review|survey", tolower(hub_title))) {"REVIEW"} else {"ARTICLE"} #Whether it is a review by the title
                        hub_ID        <- y
                        row           <- data.frame(cluster, cluster_code,
