...
columns_in_myDataCorrect <- intersect(c("X_C", "cluster_code",
                                        ...
                                        "AU", "PY", "DI", "TI", "Z9", "X_E", "DE", "SO", "VL", "IS", "WC", "Country", "UT"),
...
change_names <- c("AU",       "PY",   "DI",  "TI",       "Z9",                 "X_E",     "DE",              "SO",        "VL",  "IS",    "WC",         "UT")
paper_names  <- c("Author",   "Year", "DOI", "Title",    "Citations received", "Degree",  "Author keywords", "Journal",   "Vol", "Issue", "Categories", "ID")
...
#article_report <- article_report %>% group_by(X_C) %>% top_n(20, Degree)

# Write the article report
write.csv(article_report, file=PROJECTarticlereport, row.names = FALSE)
