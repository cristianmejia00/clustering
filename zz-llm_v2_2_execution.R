...
for (i in c(1:nrow(oldest_data))) {
  ...
  old_summary <- ask_llm(system_prompt = prompt_old$system,
...
for (cluster_code in list_of_cluster_codes) {
  ...
  while(!cluster_completed) {
    tmp <- tryCatch({
      cluster_description <- ask_llm(system_prompt = prompt_desc$system,
  ...
  while(!cluster_completed) {
    tmp <- tryCatch({
      ...
      cluster_name <- ask_llm(system_prompt = prompt$system, 
...
for (cluster_code in list_of_cluster_codes) {
  ...
  while(!cluster_completed) {
    tmp <- tryCatch({
      ...
      cluster_description <- ask_llm(system_prompt = prompt_enh$system,
